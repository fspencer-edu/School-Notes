...
- Data is encapsulated and send, no error recovery, flow control, or congestion control is performed
- The rate of traffic to the router is $\lambda_{in}$ bytes/sec
- Packets from A to B pass through a router and over a shared outgoing like of capacity R

<img src="/images/Pasted image 20251218105731.png" alt="image" width="500">

<img src="/images/Pasted image 20251218105809.png" alt="image" width="500">

- Graph (a) plots the per-connection throughput (bytes per second at the receiver) as a function of the connection-sending rate
- Upper limits on throughput is R/2

<img src="/images/Pasted image 20251218105936.png" alt="image" width="500">

- When the sending rate exceeds R/2, the average number of queues packets in the router is unbounded, the average delay between source and designation become infinite
- Large queuing delays are experienced as the packet-arrival rate near the link capacity

**Scenario 2: Two Senders and a Router with Finite Buffers**

<img src="/images/Pasted image 20251218110223.png" alt="image" width="500">

- Packets will be dropped if there is an already-full buffer
- The rate at which the transport layer sends segments into the network is referred to as the offered load to the network
- Performance is dependent on retransmission

<img src="/images/Pasted image 20251218110330.png" alt="image" width="500">

- a
	- Ideal case, where sender knows when buffer is free
- b
	- Sender retransmits only when a packet is known for certain to be lost
- c
	- Sender may time out prematurely and retransmit a packet that has been delayed in queue but not lost
	- Both the original data packet and retransmission may reach the receiver
	- Packet is forwarded twice, the throughput will have an asymptotic value of R/4 as the offered load approaches R/2

**Scenario 3: Four Senders, Routers with Finite Buffers, and Multihop Paths**

<img src="/images/Pasted image 20251218110725.png" alt="image" width="500">

- Host A to Host C, passing through routers R1 and R2

<img src="/images/Pasted image 20251218110846.png" alt="image" width="500">

- Traffic from A-C and B-D must compete at router R2 for limited amount of buffer space
- Whenever a packet is dropped at a second-hop router, the work done by the first-hop router in forwarding a packet to the second-hop router ends up being wasted
- Give priority to packets that have already traversed some number of upstream routers

### Approaches to Congestion Control

- End-to-end congestion control
	- Network provides no explicit support to the transport layer for congestion-control purposes
	- TCP segment loss is taken as an indication of network congestion, and TCP decreases it window size
- Network-assisted congestion control
	- Routers provide explicit feedback to the sender and/or receiver regarding the congestion state of the network
	- ATM Available Bite Rate (ABR) congestion control, a router informs the sender of maximum host sending rate it can support

- For network assisted congestion control, congestion info is relayed in two ways
	- Direct feedback from network router to the sender
		- Choke packet
	- Router marks/updates a field in packet flowing from sender to receiver to indicate congestion
	- Takes full round time

<img src="/images/Pasted image 20251218111436.png" alt="image" width="500">


## TCP Congestion Control

- Each side of a TCP connection consists of a receiver buffer, a send buffer, and several variables
- TCP keeps track of the sender congestion window
	- `cwnd`, imposes a constraint on the rate at which a TCP sender can send traffic into the network
- TCP receiver buffer is so large that the receive-window constraint can be ignored
- The constraint limits the amount of acknowledged data at the sender and therefore indirectly limits the sender's send rate
- If acknowledgements arrive at a slow rate, then the congestion window will be increased at a slow rate, if increased, then the window will increase
- TCP is self clocking, since it uses acknowledgements to trigger its increase in congestion window size
...
- Slow start
	- The value of `cmwd` is initialized to a small value of 1 MSS, resulting in an initial sending rate of MSS/RTT
		- MSS = 500 bytes
		- RTT = 200 msec
		- The initial sending rate is about 20 kbps
	- The TCP sender would like to fine the amount of available bandwidth
	- `cwmd` increases by 1 MSS every time a transmitted segment is first acknowledged
	- If there is a loss event, the value is set to 1, and the process starts again
	- Sets the value of a second state variable, `ssthresh` to `cwnd/2`

- Congestion Avoidance
	- TCP adopts a more conservative approach and increases the value of `cwnd` by a single MSS every RTT
	- 



<img src="/images/Pasted image 20251218222140.png" alt="image" width="500">
...
<img src="/images/Pasted image 20251218222613.png" alt="image" width="500">

**TCP Congestion Control: Retrospective**
- TCP's congestion control consists of linear (additive) increase in `cwnd` and then a halving on a triple duplicate-ACK event
- TCP congestion control is referred to as additive-increase, multiplicative-decrease (AIMD)
- Gives rise to "saw tooth" behaviour
- TCP linearly increases it congestion window size until a ACK event

#### TCP Splitting: Optimizing the Performance of Cloud Services

- For a query, the server requires 3 TCP windows during slow start to deliver the response
- To improve user-perceived performance
	- Deploy front=end servers closer to the user
	- Utilize TCP spitting by breaking the TCP connection at the front-end server
		- Client establishes a TCP connection to the nearby front-end, and the front-end maintains a persistent TCP connection to the data centre with a very large TCP congestion window
		- Reduces TCP retransmission delays

- Early version of TCP, known as TCP Tahoe, unconditionally cut its congestion window to 1 MSS and entered the slow-start phase after either a timeout-indicated or tripled-duplicate-ACK-indicated loss event
- The newer version, TCP Reno, incorporated fast recovery

<img src="/images/Pasted image 20251218223144.png" alt="image" width="500">

<img src="/images/Pasted image 20251218223213.png" alt="image" width="500">


**Macroscopic: Description of TCP Throughput**
- During a round-trip interval, the rate at which TCP sends data is a function of the congestion window and the current RT

**TCP Over High-Bandwidth Paths**

### Fairness

- A bottleneck link, is where there is a congested path, with other paths that have abundant transmission capacity and are not congested
- Congestion-control mechanism is said to be fair if the average transmission rate of each connection is R/K

<img src="/images/Pasted image 20251218223850.png" alt="image" width="500">

<img src="/images/Pasted image 20251218223900.png" alt="image" width="500">

- If TCP is to share the link equally, the realized throughput should fall along the 45-degree arrow from origin
- Ideally, the sum of the two throughputs should equal R
- In reality, client-server applications can obtain unequal portions of link bandwidth

**Fairness and UDP**
- Possible for UDP sources to crowd out TCP traffic

**Fairness and Parallel TCP Connections**
- Web browsers use multiple parallel TCP connections
	- Gets a larger fraction of the bandwidth in a congested link

### Explicit Congestion Notification (ECN): Network-assisted Congestion Control

<img src="/images/Pasted image 20251218224333.png" alt="image" width="500">

- TCP sender received no explicit congestion indications from the network layer, and instead infers congestion through observed packet loss
- Explicit Congestion Notification
	- Extensions to IP and TCP that allow the network to explicitly signal congestion to a TCP sender and receiver
	- 2 bits in the Type of Service field of the IP datagram are used for ECN

## Summary

- Reliable data transfer can be provided by link, network, transport, or application layer protocol
- Protocols can also implement acknowledgements, timers, retransmissions, and sequence numbers
- Datagram Congestion Control Protocol (DCCP) provides a low-overhead, message-oriented, UDP-like unreliable service, but an application-selection form of congestion control that is compatible with TCP
- Google's QUIC (Quick UDP Internet Connections) protocol, implemented in Google's Chromium browser, provide reliability via retransmission, error correction, fast-connection setup, and rate-based congestion control
- DCTCP (Data Centre TCP) is a version of TCP designed for data centre networks
- Stream Control Transmission Protocol) is a reliable, message-oriented protocol that allows several different application-level streams to be multiplexed through a single SCTP connection